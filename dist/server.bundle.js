...
var Space_Separator, ID_Start, ID_Continue, unicode, util, source, parseState, stack, pos, line, column, token, key, root, parse3, lexState, buffer, doubleQuote, sign, c, lexStates, parseStates, stringify, JSON5, lib, dist_default;
var init_dist = __esm({
  "node_modules/json5/dist/index.mjs"() {
    Space_Separator = /[\u1680\u2000-\u200A\u202F\u205F\u3000]/;
    ...
    parse3 = /* @__PURE__ */ __name(function parse4(text, reviver) {
      source = String(text);
      parseState = "start";
      stack = [];
      pos = 0;
...
var require_jsoncSort = __commonJS({
  "src/jsoncSort.js"(exports2, module2) {
    "use strict";
    var jsoncParser = (init_main2(), __toCommonJS(main_exports2));
    var { buildKeyOrderer, buildComparator } = require_sort();
    ...
    function rewriteObject(node, ctx, depth) {
      ...
      const propsRaw = computePropertySpans(children, source2, openIdx + 1, closeIdx);
    ...
    function rewriteArray(node, ctx, depth) {
      ...
      if (!sortOrder || isIdentityOrder(sortOrder)) {
        return rewriteArrayElementsInPlace(source2, openIdx, closeIdx, children, ctx, depth);
      }
      const elemSpans = computeItemSpans(children, source2, openIdx + 1, closeIdx);
      const rewrittenElems = children.map(/* @__PURE__ */ __name(function eachElem(elNode, idx) {
    ...
    function rewriteArrayElementsInPlace(source2, openIdx, closeIdx, children, ctx, depth) {
      let out = "";
    ...
    __name(rewriteArrayElementsInPlace, "rewriteArrayElementsInPlace");
    function computeArraySortOrder(children, options) {
      if (options.sortArrays && children.every(isStringNode)) {
        const cmp = buildComparator(options);
        return stableSortIndices(children, /* @__PURE__ */ __name(function strCmp(a, b) {
          return cmp(a.value, b.value);
        }, "strCmp"));
      }
      if (options.sortNumberArrays && children.every(isNumberNode)) {
        const order = options.sortOrder === "desc" ? -1 : 1;
        return stableSortIndices(children, /* @__PURE__ */ __name(function numCmp(a, b) {
      ...
      if (options.sortObjectArraysBy && children.every(isObjectNode)) {
        ...
        const cmp = buildComparator(options);
        const order = options.sortOrder === "desc" ? -1 : 1;
        return stableSortIndices(children, /* @__PURE__ */ __name(function objCmp(a, b) {
          const av = objectPropertyLeaf(a, key2);
          const bv = objectPropertyLeaf(b, key2);
          if (av == null && bv == null) return 0;
          if (av == null) return 1;
          if (bv == null) return -1;
          if (typeof av === "string" && typeof bv === "string") return cmp(av, bv);
    ...
    __name(computeArraySortOrder, "computeArraySortOrder");
    function isStringNode(n) {
      return n.type === "string";
    }
    __name(isStringNode, "isStringNode");
    function isNumberNode(n) {
      return n.type === "number";
    }
    __name(isNumberNode, "isNumberNode");
    function isObjectNode(n) {
      return n.type === "object";
    }
    __name(isObjectNode, "isObjectNode");
    function objectPropertyLeaf(objNode, key2) {
      const props = objNode.children || [];
      for (const prop of props) {
        const keyNode = prop.children && prop.children[0];
        const valueNode = prop.children && prop.children[1];
        if (keyNode && keyNode.value === key2 && valueNode) {
          return valueNode.type === "string" || valueNode.type === "number" ? valueNode.value : void 0;
        }
      }
      return void 0;
    }
    __name(objectPropertyLeaf, "objectPropertyLeaf");
    function stableSortIndices(children, nodeCmp) {
      const indices = children.map(/* @__PURE__ */ __name(function eachIdx(_, i) {
        return i;
      }, "eachIdx"));
      indices.sort(/* @__PURE__ */ __name(function stableCmp(a, b) {
        const r = nodeCmp(children[a], children[b]);
        return r !== 0 ? r : a - b;
      }, "stableCmp"));
      return indices;
    }
    __name(stableSortIndices, "stableSortIndices");
    ...
    __name(isIdentityOrder, "isIdentityOrder");
    ...
    function computeItemSpans(items, source2, bodyStart, bodyEnd) {
      const spans = [];
      ...
      for (let idx = 0; idx < items.length; idx++) {
        ...
        let cursor = prevEnd;
        if (idx > 0) {
          while (cursor < itemNode.offset && (source2[cursor] === " " || source2[cursor] === "	")) cursor++;
          if (source2[cursor] === ",") cursor++;
        }
        const start = absorbLeadingComments(source2, cursor, itemNode.offset);
        const valueEnd = itemNode.offset + itemNode.length;
        const text = source2.slice(start, valueEnd);
        const hardEnd = idx === items.length - 1 ? bodyEnd : items[idx + 1].offset;
        const trailing = scanTrailingComment(source2, valueEnd, hardEnd);
        const trailingHasNewline = /\n$/.test(source2.slice(start, trailing.end));
        spans.push({
          text,
          start,
    ...
    __name(computeItemSpans, "computeItemSpans");
    function scanTrailingComment(source2, valueEnd, hardEnd) {
      let i = valueEnd;
      ...
      if (source2[j] === "/" && (source2[j + 1] === "/" || source2[j + 1] === "*")) {
        let k = j;
        if (source2[j + 1] === "/") {
          while (k < hardEnd && source2[k] !== "\n") k++;
        } else {
          while (k < hardEnd - 1 && !(source2[k] === "*" && source2[k + 1] === "/")) k++;
          k += 2;
        }
        return { spacing: source2.slice(wsStart, j), comment: source2.slice(j, k), end: k };
      }
      return { spacing: "", comment: "", end: valueEnd };
    }
    __name(scanTrailingComment, "scanTrailingComment");
    function computePropertySpans(props, source2, bodyStart, bodyEnd) {
      const spans = computeItemSpans(props, source2, bodyStart, bodyEnd);
      return spans.map(/* @__PURE__ */ __name(function addKey(span, idx) {
    ...
    __name(stripLeadingSeparator, "stripLeadingSeparator");
...
var require_codeActions = __commonJS({
  "src/codeActions.js"(exports2, module2) {
    ...
    __name(isTextSortable, "isTextSortable");
